...
if hash qemu-system-i386 2>/dev/null; then
    ...
    qemu-system-i386 -s -S -kernel chernos.elf
else
    echo "Could not find qemu-system-i386"
    if hash qemu-system-x86_64 2>/dev/null; then
        echo "Found qemu-system-x86_64, can't use it for debugging because of architecture mismatch"
    fi
fi
